...
public class Chassis extends SubsystemBase {
  ...
  public void drive(ChassisSpeeds chassisSpeeds){
    ...
    if(Math.abs(forwardXSpeed)>Constants.Chassis.MAX_SPEED_ms2){
      ...
      left1.set(leftVelocity);
      right1.set(rightVelocity);
    }
  ...
  public Command driveCommand(XboxController controller){ 
    return Commands.run(
      () -> 
        this.arcadeDrive(
        (controller.getRawAxis(Constants.Controller.ID_JOYSTICK_SPEED)- controller.getRawAxis(Constants.Controller.ID_JOYSTICK_BRAKE)),
        (Math.abs(controller.getRawAxis(Constants.Controller.ID_JOYSTICK_ROT)) > Constants.Chassis.kDeadBandRot ? controller.getRawAxis(Constants.Controller.ID_JOYSTICK_ROT) : 0)), 
      this);
  }
}
